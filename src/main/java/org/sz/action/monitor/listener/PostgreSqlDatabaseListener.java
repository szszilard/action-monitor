...
public class PostgreSqlDatabaseListener implements DatabaseListener {
    ...
    public PostgreSqlDatabaseListener(@Autowired Connection connection, @Autowired ActionMessageSender actionMessageSender, @Value("${action-monitor.topic}") String topic) {
    ...
    private void handleNotification(PGNotification pgNotification) throws IOException {
        ...
        String actionMessageChannel = pgNotification.getName();
        logger.info("Notification received on channel [{}] with payload [{}]", actionMessageChannel, notificationParameter);
        Notification notification = new ObjectMapper().readValue(notificationParameter, Notification.class);
        String actionMessageText = createActionMessageFrom(notification);
        ActionMessage actionMessage = new ActionMessage(actionMessageText, actionMessageChannel);
